...
import { useContext, MutableRefObject, useRef, useEffect, useState, FormEvent, FC } from "react";
...
const EditLessonForm: FC<Props> = ({ lessonId }) => {
  ...
  return (
    <Container>
      <div>
        <Form name="articleForm" ref={editLessonForm} onSubmit={e => editLesson(e)}>
          <Form.Group>
            <Form.Label className={`text-${theme === "dark" ? "light" : "dark"}`} htmlFor="exampleFormControlInput1">Title</Form.Label>
            <Form.Control type="text" onChange={e => setTitle(e.target.value)} defaultValue={lesson?.title} className="form-control" id="exampleFormControlInput1" placeholder="name@example.com" />
          </Form.Group>
          ...
          <Form.Group>
            ...
            <Form.Select name="courses" onChange={e => setCourse(e.target.value)}>
              <option value={lesson?.courseId} style={{ display: 'none' }}>default</option>
          ...
          <Form.Group>
            ...
            <textarea className="form-control" onChange={e => setDescription(e.target.value)} defaultValue={lesson?.description} name="description" id="exampleFormControlTextarea1" rows={3}></textarea>
          </Form.Group>

          <Form.Group>
            <label className={`text-${theme === "dark" ? "light" : "dark"}`} htmlFor="exampleFormControlTextarea1">Lesson Content</label>
            <textarea className="form-control" onChange={e => setContent(e.target.value)} defaultValue={lesson?.courseContent} name="content" id="exampleFormControlTextarea1" rows={15}></textarea>                   
          </Form.Group>
