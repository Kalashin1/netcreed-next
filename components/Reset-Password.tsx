...
const ResetPassword: FC = () => {
  ...
  const ResetPasswordForm: MutableRefObject<null | HTMLFormElement> = useRef(null);

  const resetPassword = async (e: FormEvent<HTMLFormElement>, form: HTMLFormElement) => {
    setShowSpinner(true);
    try {
  ...
  return (
    <>
      <Form ref={ResetPasswordForm} name="resetPassword"
        onSubmit={e => resetPassword(e, ResetPasswordForm.current!)}>
        <Form.Group className="mb-3" controlId="password">
          <Form.Label>Email address</Form.Label>
          <Form.Control id="password" type={inputType} name="password" required placeholder="Password" />
        ...
        <Form.Group className="mb-3" controlId="password_2">
          <Form.Label>Email address</Form.Label>
          <Form.Control type={inputType} id="password_2" name="password_2" required placeholder="Password" />
          <Form.Text className="text-muted" style={{ cursor: 'pointer' }} onClick={toggleShowPassword}>
            show password
          </Form.Text>
        </Form.Group>
