...
const AlikePost: FC<Articles> = ({ articles }) => {
  ...
  return (
    <Container className="pt-4 pb-4">
      <h5 className={`font-weight-bold spanborder text-${theme === 'light'? 'dark': 'light'}`}><span>Read next</span></h5>
      <Row>
        <Col lg={6}>
          { [articles[0], articles[5] ].map((article, index) => (
            <Card bg={theme} className={`border-0 mb-4 box-shadow text-${theme === 'light'? 'dark': 'light'}`} key={index}>
        ...
        <Col lg={6}>
          <div className="flex-md-row mb-4">
            {articles && articles.slice(1, 3).map((article, index) => (
              <Card bg={theme} className={`p-4 my-4 text-${theme === 'light'? 'dark': 'light'}`} key={index}>
